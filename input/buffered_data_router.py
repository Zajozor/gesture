...
#     """
#     def __init__(self, input_parser_instance: SerialPortParser, length=200, sensor_count=5,
#                  signal_canvas_instance: SimpleCanvas = None, log_counts=False, frequency=60):
...
#                     self.signal_canvas_instance.add_new_signals(
...
#     def update(self):
#         while self.running:
#             has_new_data = self.input_parser_instance._data_changed[:]
...
#     canvas = SimpleCanvas(2, 3, length=200)
#     # ip = InputParser('/dev/cu.SLAB_USBtoUART')
#     ip = SerialPortParser('/dev/ttys002')
#     ip.start(threaded=True)
#
#     dr = DataRouter(ip, signal_canvas_instance=canvas, log_counts=True)
